...
const StatusBadge = styled.span`
  ...
  background: ${props => {
    switch(props.status) {
      case 'pago': return 'rgba(16, 185, 129, 0.1)';
  ...
  border: 1px solid ${props => {
    switch(props.status) {
      case 'pago': return 'rgba(16, 185, 129, 0.2)';
...
const CommissionsSpreadsheet = ({ 
  ...
  const stats = {
    total: commissions.reduce((sum, c) => sum + parseFloat(c.comissao), 0),
    pendentes: commissions.filter(c => c.status === 'pendente').length,
    pagas: commissions.filter(c => c.status === 'pago').length,
    valorTotal: commissions.reduce((sum, c) => sum + parseFloat(c.valor), 0),
  };
    ...
    return (
      <Container>
        <div style={{ textAlign: 'center', padding: '40px' }}>
          <div style={{ color: '#a0a0a0' }}>Carregando comissões...</div>
        </div>
      </Container>
    );
  }
    ...
    <Container>
      <Header>
        <HeaderLeft>
          ...
          <div>
            <HeaderTitle>💰 Comissões</HeaderTitle>
            <HeaderSubtitle>Gestão de comissões e vendas</HeaderSubtitle>
          </div>
        </HeaderLeft>
        <HeaderRight>
          <ActionButton 
      ...
      <StatsGrid>
        ...
        >
          ...
          <StatSubtitle>Aguardando pagamento</StatSubtitle>
        </StatCard>
        ...
        >
          ...
          <StatValue>{stats.pagas}</StatValue>
          <StatSubtitle>Já recebidas</StatSubtitle>
        </StatCard>
...
export default CommissionsSpreadsheet;
